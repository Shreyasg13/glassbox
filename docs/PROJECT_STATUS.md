...
| 7 | **First-time-visitor onboarding tour** | `WelcomeTour.tsx`: a plain modal carousel (icon/title/blurb/"take me there" link/dot pagination), shown once per browser via localStorage, walking a new user through Dashboard → My Agents → Reports (+ an Admin slide for admin accounts). Deliberately **not** an LLM agent -- a walkthrough of this app's own fixed UI has nothing to gain from a live model call (extra cost/latency for a result that should be identical every time) | Done | ✅ Done |
| 8 | **Report charts** -- `/reports/[id]` is LLM-narrated prose only, no risk/signal visualizations | Not started | Add real charts (risk/signal) to the daily report narrative pages | 📋 Planned |
