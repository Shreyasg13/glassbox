...
def test_bootstrap_creates_controls_profiles_and_benchmarks(fake):
    ...
    assert r["accounts"] == len(ids) == 13  # 9 controls + 2 profiles + 2 benchmarks
...
def test_dry_run_saves_nothing(fake):
    ...
    assert r["dry_run"] and r["accounts"] == 13
...
def test_api_run_needs_bootstrap_then_bootstraps_once_then_runs_daily(client, fake, monkeypatch):
    ...
    assert r.status_code == 200 and r.json()["accounts"] == 13
...
def test_api_overview_account_and_signals_after_a_run(client, fake, monkeypatch):
    ...
    assert ov["initialised"] and len(ov["accounts"]) == 13
...
def test_the_real_registry_builds_all_31_accounts_on_a_full_universe_with_no_users(fake, monkeypatch):
    ...
    assert r["accounts"] == len(f.accounts) == 9 + 11 * 2  # controls + (profile + benchmark) per registry entry
